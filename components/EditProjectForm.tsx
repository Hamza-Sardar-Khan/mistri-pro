...
export default function EditProjectForm({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    getProjectById(projectId).then((project) => {
      ...
      project.imageUrls?.forEach((url: string, i: number) =>
        existing.push({ id: `img-${i}`, url, preview: url, type: "image" })
      );
      project.videoUrls?.forEach((url: string, i: number) =>
        existing.push({ id: `vid-${i}`, url, preview: url, type: "video" })
      );
      project.audioUrls?.forEach((url: string, i: number) =>
        existing.push({ id: `aud-${i}`, url, preview: "", type: "audio" })
      );
  ...
  const addMedia = (e: React.ChangeEvent<HTMLInputElement>, type: "image" | "video" | "audio") => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const existingImages = mediaFiles.filter((m) => m.type === "image" && m.url).map((m) => m.url!);
      const existingVideos = mediaFiles.filter((m) => m.type === "video" && m.url).map((m) => m.url!);
      const existingAudios = mediaFiles.filter((m) => m.type === "audio" && m.url).map((m) => m.url!);

      const newFiles = mediaFiles.filter((m) => m.file);
      const imageUrls = [...existingImages];
      const videoUrls = [...existingVideos];
      const audioUrls = [...existingAudios];
      ...
      for (let i = 0; i < newFiles.length; i++) {
        ...
        if (newFiles[i].type === "image") imageUrls.push(data.url);
        else if (newFiles[i].type === "video") videoUrls.push(data.url);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#f5f7fa] flex items-center justify-center">
        <div className="h-8 w-8 animate-spin rounded-full border-4 border-[#0d7cf2] border-t-transparent" />
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#f5f7fa]">
      <main className="mx-auto max-w-2xl px-4 py-8">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8">
          <h2 className="text-2xl font-extrabold text-[#0e1724] mb-1">Edit Project</h2>
          <p className="text-sm text-[#97a4b3] mb-8">Update your project details</p>
                ...
                <div className="space-y-2">
                  {audios.map((m) => (
                    <div key={m.id} className="flex items-center gap-2 rounded-lg border border-gray-200 p-2">
                      ...
                      <span className="text-sm text-[#5e6d80] truncate flex-1">
                        {m.file ? m.file.name : m.url ? `Audio file` : "Audio"}
                      </span>
                      <button
                        onClick={() => removeMedia(m.id)}
          ...
          <div className="flex gap-3">
            <button
              onClick={() => router.back()}
              className="flex-1 rounded-lg border border-gray-200 py-3.5 text-sm font-medium text-[#5e6d80] transition hover:bg-gray-50"
            >
              Cancel
            </button>
            <button
